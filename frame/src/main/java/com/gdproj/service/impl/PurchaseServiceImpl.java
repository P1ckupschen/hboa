...
public class PurchaseServiceImpl extends ServiceImpl<PurchaseMapper, Purchase>
    ...
    public ResponseResult updatePurchase(PurchaseVo vo) {
        ...
        if(b){
            Integer typeId = purchase.getTypeId();
            Integer Id = purchase.getPurchaseId();
            ...
            one.setFlowTitle(purchase.getPurchaseTitle());
    ...
    @Override
    public ResponseResult getMyPurchaseList(PageQueryDto queryVo, HttpServletRequest request) {
        //类型
        Integer type = queryVo.getType();
        //部门
        Integer departmentId = queryVo.getDepartmentId();
        ...
        if(!ObjectUtil.isEmpty(role)){
            ...
            if(!is){
                queryWrapper.eq(Purchase::getUserId, JwtUtils.getMemberIdByJwtToken(request));
            }
        }
        //排序
        if (sort.equals("+id")) {
